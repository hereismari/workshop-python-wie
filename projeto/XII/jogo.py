...
def salvar_em_arquivo(j1, j2, res, i, f):
    historico = open('historico.txt', 'a+')
    historico.write('%s, %s, %s, %s, %s\n' % (j1, j2, res, i, f))
    historico.close()
    print 'Resultado salvo em arquivo!'
...
def iniciar_jogo(matriz):
    ...
    salvar_em_arquivo(nome_jogador1, nome_jogador2,
                      resultado, inicio_jogo, fim_jogo)

    print
    novo_jogo = raw_input('Deseja jogar novamente? (S/N): ')
